...
Ulog <- function(alpha, mu, gamma, Sigma){
  num <- log(1+t(alpha)%*%mu) - gamma*t(alpha)%*%Sigma%*%alpha
  denom <- (2*(1+t(alpha)%*%mu)^2)
  ulog <- num/denom
  
  return(as.numeric(ulog))
}

# Añadiendo un parámetro de penalización para la restricción sum(alpha)=1
Ulog_penal <- function(alpha, mu, gamma, Sigma, penal){
  if(t(alpha)%*%mu-1<=0){
    return(-2000 + sign(1+alpha%*%mu)*(1+alpha%*%mu)^4)
  } else {
    ulog <- Ulog(alpha,mu,gamma,Sigma)
    penal <- penal*(sum(alpha)-1)^2
    return(as.numeric(ulog - penal))}
}

eval_f <- function(y){
  a2 <- (log(y) -1)*(1/mu_hat[1,])
  -Ulog_penal(a2, mu_hat[1,], 1, getSigmaMV(se_hat[1,], Xtrain), 10^30)
}

ejemplo_optim <- function(x){
  fun <- x^2
  return(fun)
}

inicial <- rbind(alpha_3_1,alpha_3_1+rnorm(1), alpha_3_1+rnorm(1, sd=0.4))
a <- DEoptim(eval_f, lower=rep(1e-6,5), upper=rep(100,5), DEoptim.control(trace = FALSE))
summary(a)
a_conv <- (log(a$optim$bestmem)-1)*(1/mu_hat[1,])
#res <- optim(par=a_conv, fn = eval_f, method = "BFGS")
print(a_conv)
#print(res)
```
...
# Prueba con una función sencilla
ejemplo_optim <- function(x,y,z){
  fun <- y*x^2 +z*x
  return(fun)
}

# Optimizamos comprobando la penalización
Alpha_logU <- function(mu_t, Sigma, gamma_t){
  print(mu_t)
  rho <- 1
  ver_alpha <- TRUE
  while(ver_alpha){
    alpha <- nloptr(
  x0 = rep(1/5, 5),
  eval_f = function(alpha) -Ulog_penal(alpha, mu=mu_t, gamma=gamma_t, Sigma=Sigma_t, penal=rho),
  opts = opts
  )
    print('Suma alpha')
    print(sum(alpha))
...
a_log2 <- getAlpha_ts(mu_hat, se_hat, gamma_param, getSigmaMV, Alpha_logU, Xtrain, Xtest)

check_log <- getChecks(a_log, mode=c("sum1"))
ret_log <- getRet(a_log, Xtest, check_log)
print(paste("Rendimiento Final Acumulado:", round(ret_log * 100, 2), "%"))
```

```{r}
opts <- list(
  algorithm = "NLOPT_GN_MLSL",#NLOPT_GN_MLSL",#"NLOPT_GN_MLSL_LDS",
  maxeval = 2000,
  local_opts = list(
    algorithm = "NLOPT_LD_LBFGS", 
    xtol_rel = 1e-8
...
k <- getSigmaMV(se_hat[1,], Xtrain)

alpha_p <- nloptr(
  x0 = getAlphaRandom(mu = mu_hat[1,], 
                      gamma = 1,
                      Sigma = k),
  
  eval_f = function(alpha) {
    -Ulog_penal(alpha,
                mu    = mu_hat[1,],
                gamma = 1,
                Sigma = k,
...
```{r}
alpha_p <- nloptr(
  x0 = getAlphaRandom(mu=mu_hat[1,], gamma=1, Sigma=getSigmaMV(se_hat[1,],Xtrain)),
  eval_f = function(alpha){-Ulog_penal(alpha, 
                                       mu=mu_hat[1,], 
                                       gamma=1, 
                                       Sigma=getSigmaMV(se_hat[1,],Xtrain), 
                                       penal=10)},
  opts = opts,
  lb = rep(-200,5),
  ub = rep(200,5)
...
```{r}
x0 <- getAlphaRandom(mu=mu_hat[1,], gamma=1, Sigma=getSigmaMV(se_hat[1,],Xtrain))
pl <-  function(alpha){-Ulog_penal(alpha, 
                                       mu=mu_hat[1,], 
                                       gamma=1, 
                                       Sigma=getSigmaMV(se_hat[1,],Xtrain), 
                                       penal=10)}
...
Alpha_logU <- function(mu_t, Sigma, gamma_t){
  ...
  ver_alpha <- TRUE
  while(ver_alpha){
    # Primero encontramos un buen punto inicial para iterar basándonos en el rango de la solución del apartado 3.1
    alpha_0 <- getAlphaRandom(mu_t, Sigma, gamma_t)
    #alpha_0 <- rep(1/5,5)
    res <- optim(par=alpha_0, fn = function(alpha) -Ulog_penal(alpha, mu=mu_t, gamma=gamma_t, Sigma=Sigma_t, penal=rho),
                method = "BFGS")
    alpha <- res$par
    print('Suma alpha')
    print(sum(alpha))
...
a_log <- getAlpha_ts(mu_hat, se_hat, gamma_param, getSigmaMV, Alpha_logU, Xtrain, Xtest)
...
```{r}
# Obtengo alpha para la log-utilidad
## voy a asumir que la sigma se obtiene de la misma forma así que lo único que cambia es la función para obtener alpha
alpha_lu <- getAlpha_ts(
  mus = mu_hat, 
  sigs = se_hat, 
  gamma = gamma_param,
  getSigmaFunc = getSigmaMV,   
  getAlphaFunc = Alpha_logU,
  Xtrain = Xtrain, 
  Xtest = Xtest
)
getAlphaRandomPos(mu_hat, se_hat, gamma_param)
```
