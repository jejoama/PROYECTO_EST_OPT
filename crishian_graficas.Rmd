...
getPred <- function(x_train, x_test_past){
  ...
  mod <- auto.arima(x_past, stepwise = FALSE, approximation = FALSE)
  fc <- forecast(mod, h = 1)
...
#Gráfica 1: Real vs Predicho (activo)
```{r}
library(ggplot2)
...
for(i in 1:5){
  ...
  print(
    ggplot(df, aes(t)) +
      ...
      labs(title=paste("Real vs Predicho - Activo", i),
           x="Tiempo Test", y="Retorno") +
      theme_minimal()
  )
}
...
#Gráfica 2: Volatilidad pronosticada σ̂_t (activo 1)
```{r}
library(ggplot2)
...
for(i in 1:5){
  ...
  print(
    ggplot(df, aes(x = t, y = sigma)) +
      geom_line(size = 1, color = "red") +
      labs(
        title = paste("Volatilidad Pronosticada - Activo", i),
        x = "Tiempo Test",
        y = expression(sigma[hat]~(t))
      ) +
      theme_minimal()
  )
}
...
#Gráfica 3: Heatmap / Matriz de pesos α_t
```{r}

library(ggplot2)
library(reshape2)

df_alpha <- data.frame(alpha_3_1)
df_alpha$time <- 1:nrow(df_alpha)
df_melt <- melt(df_alpha, id="time")

ggplot(df_melt, aes(time, variable, fill=value)) +
  geom_tile() +
  scale_fill_gradient2(low="red", high="blue", mid="white") +
  labs(title="Evolución de pesos α_t (MV con cortos)",
       x="Tiempo", y="Activo")
...
#Gráfica 4: Peso promedio por activo
```{r}
barplot(colMeans(alpha_3_1),
        main="Pesos Promedios - MV con Cortos",
        ylab="Peso Promedio",
        names.arg=paste("A",1:5))
...
#Gráfica 5: Heatmap pesos sin cortos
```{r}
df_alpha <- data.frame(alpha_3_2)
df_alpha$time <- 1:nrow(df_alpha)
df_melt <- melt(df_alpha, id="time")
...
ggplot(df_melt, aes(time, variable, fill=value)) +
  ...
  labs(title="Pesos α_t - MV Sin Cortos", x="Tiempo", y="Activo")


```


#Gráfica 6: Pesos del portafolio con utilidad log

```{r}
df_alpha <- data.frame(alpha_log)
df_alpha$time <- 1:nrow(df_alpha)
df_melt <- melt(df_alpha, id="time")

ggplot(df_melt, aes(time, variable, fill=value)) +
  geom_tile() +
  labs(title="Pesos α_t - Utilidad Log",
       x="Tiempo", y="Activo")
...
#Gráfica 7: Retorno acumulado (curva equity)

```{r}
ret_series <- 1 + rowSums(alpha_log * Xtest)
equity <- cumprod(ret_series)

plot(equity, type="l", lwd=2, col="darkgreen",
     main="Evolución del Portafolio (Utilidad Log)",
     ylab="Valor Acumulado", xlab="Tiempo Test")
