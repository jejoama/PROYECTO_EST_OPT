...
x_train <- window(train_serie, start=1, end=floor(0.8*filas_train))

# Validación: 24 ultimos
x_test <- window(train_serie, start=floor(0.8*filas_train)+1, end=filas_train)
...
############################################
# SECCIÓN 4 — UTILIDAD LOG (CON CORTOS)
############################################

# 4.1 Matriz de covarianzas: Sigma = D R D
...
# 4.2 Optimización de la utilidad logarítmica (permitiendo cortos)
getAlphaLog <- function(mu, Sigma, gamma){
  ...
  # Lagrange multiplier
  lambda <- (t(vec) %*% invSigma %*% mu - gamma) /
            (t(vec) %*% invSigma %*% vec)
...
# Solo se exige suma=1 (permitimos cortos)
...
# Utilidad log relativa (comparada con alpha igual)
Ulog_rel <- getUEval(
  alpha_log, mu_hat, se_hat,
  Xtrain, Xtest,
  gammaLog, getSigmaLog,
  check_log, Ulog
)
